from flask import Flask, send_file, request, render_template, abort, redirect, url_for, jsonify
import os
import mimetypes
...
def get_sorted_files(current_dir: Optional[Path] = None) -> list:
    ...
    file_types = {
        ".mp4": 1, ".avi": 1, ".mkv": 1, ".webm": 1,  # Videos
        ".jpg": 2, ".png": 2, ".jpeg": 2, ".gif": 2,  # Images
        ".mp3": 3, ".wav": 3, ".flac": 3,  # Audio
        ".pdf": 4, ".txt": 4, ".docx": 4,  # Documents
        "other": 6  # Everything else
    }

    def file_sort_key(filepath):
        return (file_types.get(filepath.suffix.lower(), file_types["other"]), filepath.name.lower())
...
def index(subpath=None):
    ...
    return render_template('index.html', items=items, current_path=subpath or '')
...
if __name__ == '__main__':
    app.run(host=os.getenv("ADDRESS", "0.0.0.0"), port=os.getenv("PORT", 5000), threaded=True, debug=True)